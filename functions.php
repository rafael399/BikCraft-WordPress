<?php 
	...
	function register_scripts() {
		// Desregistrar jQuery padrão do WordPress
		wp_deregister_script('jquery');

		// Registrar o novo jQuery
		wp_register_script('jquery', get_template_directory_uri() . '/js/libs/jquery-1.11.2.min.js',
			array(), '1.11.2', true);
		...
		wp_register_script('main-script', get_template_directory_uri() . '/js/main.js',
			array('jquery', 'plugins-script'), false, true);

		// Registrar Modernizr
		wp_register_script('modernizr', get_template_directory_uri() . '/js/libs/modernizr.custom.45655.js',
			array(), '45655', false);

		// Carregar scripts no site
		wp_enqueue_script('modernizr');
		wp_enqueue_script('main-script');
	}
	add_action('wp_enqueue_scripts', 'register_scripts');

	// Função para registrar CSS
	function register_css() {
		// Registrar style.css
		wp_register_style('main-style', get_template_directory_uri() . '/style.css', array(), false, false);

		// Carregar CSS no site
